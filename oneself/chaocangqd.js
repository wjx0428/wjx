...
========= 青龙 =========
变量格式：export chaocang_ck='xxxxx@xxxxx'  多个账号用 @分割 
cron: 1 0 * * *
 */
...
//////////////////////
let chaocang_ck = process.env.chaocang_ck;               // 这里是 从青龙的 配置文件 读取你写的变量
let chaocang_ckArr = [];
let data = '';
let msg = '';
...
!(async () => {
	...
	if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		...
		console.log(`\n=================== 共找到 ${chaocang_ckArr.length} 个账号 ===================`)



		for (let index = 0; index < chaocang_ckArr.length; index++) {
	...
	return new Promise((resolve) => {
		...
		$.post(url, async (error, response, data) => {     // 这是一个 get 请求 , 如果是 post  记得把这里改了 
			try {
				...
				} else if (result.code === 0) {    // 这里是根据服务器返回的数据做判断  方便我们知道任务是否完成了
...
async function Envs() {
	if (chaocang_ck) {
		if (chaocang_ck.indexOf("@") != -1) {
			chaocang_ck.split("@").forEach((item) => {
				jchaocang_ckArr.push(item);
			});
		} else {
			chaocang_ckArr.push(chaocang_ck);
		}
	} else {
		console.log(`\n 【${$.name}】：未填写变量 chaocang_ck`)
		return;
	}
