...
class UserInfo {
    ...
    async task_tasklist(name) { // 任务列表
        ...
        let s = `${host_data}&&${this.ck}&&${REQUEST_ID}&&${TIMESTAMP}&&${this.key}&&24`
    ...
    async task_articlelist(name) { // 获取文章列表
        ...
        try {
            let options = {
                method: 'GET',
                url: `${this.hostname}${host_data}` + `?channel_id=5d3fea3ab198500f695bdec4&isDiangHao=false&is_new=true&list_count=0&size=10`,
                headers: {
                    'X-SESSION-ID': `${this.ck}`,
                    'X-REQUEST-ID': REQUEST_ID,
                    'X-TIMESTAMP': TIMESTAMP,
    ...
    async task_comment(articleId) { // 新闻资讯评论
        ...
        try {
            let options = {
                ...
                form: { channel_article_id: articleId , content : '1' }
            };
            //console.log(options);
            let result = await httpRequest(options, '新闻资讯评论');
            //console.log(result);
            if (result.code == 0) {
                DoubleLog(`账号[${this.index}],新闻资讯评论成功:[` + articleId + `]`);
            } else {
                DoubleLog(`账号[${this.index}],用户查询:失败 ❌ 了呢,原因未知！`);
                console.log(result);
            }
