...
class UserInfo {
    ...
    async task_tasklist(name) { // 任务列表
        ...
        let s = `${host_data}&&${this.ck}&&${REQUEST_ID}&&${TIMESTAMP}&&${this.key}&&14`
    ...
    async task_articlelist(name) { // 获取文章列表
        ...
        try {
            let options = {
                method: 'GET',
                url: `${this.hostname}${host_data}` + `?channel_id=5cc2cc981b011b18ee37591c&isDiangHao=false&is_new=true&list_count=0&size=10`,
                headers: {
                    'X-SESSION-ID': `${this.ck}`,
                    'X-REQUEST-ID': REQUEST_ID,
                    'X-TIMESTAMP': TIMESTAMP,
