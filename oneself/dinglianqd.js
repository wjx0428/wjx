...
========= 青龙 =========
变量格式：export dinglian_ck='xxxxx@xxxxx'  多个账号用 @分割 
cron: 1 0 * * *
*/
...
//////////////////////
let dinglian_ck = process.env.dinglian_ck;               // 这里是 从青龙的 配置文件 读取你写的变量
let dinglian_ckArr = [];
let data = '';
let msg = '';
...
!(async () => {
	...
	if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		...
		console.log(`\n=================== 共找到 ${dinglian_ckArr.length} 个账号 ===================`)



		for (let index = 0; index < dinglian_ckArr.length; index++) {
 ...
 function signin(timeout = 3 * 1000) {
	return new Promise((resolve) => {
		let url = {
			url: `https://api.dingstock.top/xserver/user/check/in`,    // 这是请求的 url 可以直接用我们抓包、精简后的URL
		...
		$.post(url, async (error, response, data) => {     // 这是一个 get 请求 , 如果是 post  记得把这里改了 
			try {
				...
				if (result.err == false) {        // 这里是根据服务器返回的数据做判断  方便我们知道任务是否完成了
...
async function Envs() {
	if (dinglian_ck) {
		if (dinglian_ck.indexOf("@") != -1) {
			dinglian_ck.split("@").forEach((item) => {
				jdinglian_ckArr.push(item);
			});
		} else {
			dinglian_ckArr.push(dinglian_ck);
		}
	} else {
		console.log(`\n 【${$.name}】：未填写变量 dinglian_ck`)
		return;
	}
